...
func doInsert(db *MemDB, wg *sync.WaitGroup, n int, isRand bool, shouldSnap bool) {
	...
	for i := 0; i < n; i++ {
		...
		// w.Put(NewItem(fmt.Sprintf("%025d", val)))
		w.Put(NewItem(string(buf)))
	}
}
...
func doReplace(wg *sync.WaitGroup, t *testing.T, w *Writer, start, end int) {
	...
	for ; start < end; start++ {
		w.Delete(NewItem(fmt.Sprintf("%010d", start)))
		w.Put(NewItem(fmt.Sprintf("%010d", start)))
	}
}
